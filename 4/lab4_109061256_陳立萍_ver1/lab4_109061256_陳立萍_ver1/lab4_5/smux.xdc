set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctl[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctl[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctl[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctl[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_dp[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports clk]
set_property IOSTANDARD LVCMOS33 [get_ports rst]
set_property PACKAGE_PIN R2 [get_ports rst]
set_property PACKAGE_PIN W5 [get_ports clk]
set_property PACKAGE_PIN W7 [get_ports {ssd_dp[7]}]
set_property PACKAGE_PIN W6 [get_ports {ssd_dp[6]}]
set_property PACKAGE_PIN U8 [get_ports {ssd_dp[5]}]
set_property PACKAGE_PIN V8 [get_ports {ssd_dp[4]}]
set_property PACKAGE_PIN U5 [get_ports {ssd_dp[3]}]
set_property PACKAGE_PIN V5 [get_ports {ssd_dp[2]}]
set_property PACKAGE_PIN U7 [get_ports {ssd_dp[1]}]
set_property PACKAGE_PIN V7 [get_ports {ssd_dp[0]}]
set_property PACKAGE_PIN U2 [get_ports {ssd_ctl[0]}]
set_property PACKAGE_PIN U4 [get_ports {ssd_ctl[1]}]
set_property PACKAGE_PIN V4 [get_ports {ssd_ctl[2]}]
set_property PACKAGE_PIN W4 [get_ports {ssd_ctl[3]}]

set_property IOSTANDARD LVCMOS33 [get_ports {ssd[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[0]}]
set_property PACKAGE_PIN W4 [get_ports {ssd[3]}]
set_property PACKAGE_PIN V4 [get_ports {ssd[2]}]
set_property PACKAGE_PIN U2 [get_ports {ssd[0]}]
set_property PACKAGE_PIN U4 [get_ports {ssd[1]}]

set_property IOSTANDARD LVCMOS33 [get_ports {D[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[0]}]
set_property PACKAGE_PIN W7 [get_ports {D[7]}]
set_property PACKAGE_PIN W6 [get_ports {D[6]}]
set_property PACKAGE_PIN U8 [get_ports {D[5]}]
set_property PACKAGE_PIN V8 [get_ports {D[4]}]
set_property PACKAGE_PIN U5 [get_ports {D[3]}]
set_property PACKAGE_PIN V5 [get_ports {D[2]}]
set_property PACKAGE_PIN U7 [get_ports {D[1]}]
set_property PACKAGE_PIN V7 [get_ports {D[0]}]
